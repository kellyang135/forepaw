...
class RolloutScorer:
    ...
    def score(
        ...
        *,
        ...
        terminal_distance = _distance(rollout.states[-1].robot.xy, goal_xy)
        ...
        effort = sum(
            action.forward_mps**2 + action.yaw_rate_rps**2
        ...
        total = (
            weights.goal_distance * terminal_distance
...
def _command_change(
    actions: tuple[ActionBlock, ...], previous_action: ActionBlock | None
) -> float:
    if not actions:
        return 0.0
    ...
    if not comparisons:
        return 0.0
    return sum(
        (right.forward_mps - left.forward_mps) ** 2
        + (right.yaw_rate_rps - left.yaw_rate_rps) ** 2
