...
async function listRuns() {
  try { const r = await fetch('/api/runs', { cache: 'no-store' }); return r.ok ? await r.json() : []; } catch { return []; }
}
...
function updateButtons() {
  ...
  $('liveTag').textContent = END ? `complete · ${END.blocks} blocks` : S.status === 'waiting' ? 'waiting for controller' : `live · ${BLOCKS.length} blocks so far`;
}
...
setInterval(async () => {
  if (END || !$('follow').checked) return;
  try { ingest(await fetchRecords(LOG)); updateButtons(); } catch { /* controller still writing */ }
}, 1000);
